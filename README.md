...
```
> 如需中文高质量向量（会额外安装 torch，约 1.5GB）：
> `.venv\Scripts\python.exe -m pip install -e ".[bge]"` 并在 `.env` 设 `EMBEDDING_PROVIDER=local_bge`。
...
### 3. 生成示例数据
```bash
.venv\Scripts\python.exe -m ecommerce_ai.data.sample_data
```
...
## 用 PyCharm 打开运行
1. `File → Open` 选择 `E:\dianshangAI` 目录。
2. PyCharm 通常会自动识别 `.venv`；若没有，`Settings → Project → Python Interpreter` 选择 `.venv\Scripts\python.exe`。
